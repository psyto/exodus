...
/**
 * Screen a wallet for sanctions/PEP before JPY conversion.
 * Integrates Fabrknt Off-Chain Compliance with Exodus's
 * KYC-gated deposit flow.
 */
...
/**
 * Check a JPY/USDC conversion for compliance across FSA/MAS jurisdictions.
 */
export async function checkConversionCompliance(params: {
  ...
  try {
    const result = await getClient().checkTransaction({
      ...
      senderWallet: params.userWallet,
    ...
    console.error("Conversion compliance check failed:", error);
